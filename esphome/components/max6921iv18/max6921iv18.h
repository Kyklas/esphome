...
class MAX6921IV18Component : public PollingComponent
{
  public:
	...
	uint8_t strftime(uint8_t pos, const char *format, ESPTime time)
		__attribute__((format(strftime, 3, 0)));

	/// Evaluate the strftime-format and print the result at position 0.
	uint8_t strftime(const char *format, ESPTime time)
  ...
  protected:
	...
	void set_digit(uint8_t idx, uint8_t val, uint8_t seg);
