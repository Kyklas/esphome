...
void MAX6921IV18Component::set_digit(uint8_t idx, uint8_t val)
{
	int g = !digits_active;
	MAX6921IV18_Digit *d = &digits[g][idx];
	d->xfer.tx_buffer = (void *)((uint32_t)((TubeDigit(val) << 16) |
											((1 << idx + 8) & 0xFF00)));
...
uint8_t MAX6921IV18Component::print(uint8_t start_pos, const char *str)
{
	...
	for (int d = start_pos; d < 8 && str[d] != '\0'; d++)
	{
		set_digit(7 - d, str[d - start_pos]);
	}
	display();
	return 0;
}
...
uint8_t MAX6921IV18Component::strftime(const char *format, time::ESPTime time)
{
